...
float frequency[9]={700,400,300,150,70,30,10,5,1};//millisec for the frequency of the bombs

float arrayRecordStampa[5];//arrays for print each digit
float arrayRecord[5];// Record's array

int randNumber;// Rand number for generate the bomb
...
void loopGame()
{
  ...
  do {
    ...
    if (p.z > ts.pressureThreshhold && (p.x - oldposition.x) < 60 && (p.y - oldposition.y) < 60)//Condition for the Cat's drawing
    {
      ...
      if ((unsigned long)(curTimeCat - preTimeCat) >= 150)
      {
